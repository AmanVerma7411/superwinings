...
export default function Quizcardone({ onBack, quizData = [], loading }) {
  ...
  if (loading) return <p className="text-white text-center">Loading quiz...</p>;
  ...
  if (!currentQ) return <p className="text-white text-center">Invalid question data.</p>;
  ...
  const handleAnswer = (selectedIndex) => {
    if (selectedIndex === currentQ.correctIndex) {
      handleNext(); // correct → next question
    } else {
  ...
  return (
    <div className="w-full max-w-xl mx-auto bg-gradient-to-r from-pink-700 via-purple-700 to-blue-800 text-white rounded-2xl shadow-lg p-6">
      ...
      <div className="relative flex justify-center mb-6">
        ...
        <span className="absolute inset-0 flex items-center justify-center text-lg font-bold">{time}</span>
      </div>
      ...
      <div className="grid grid-cols-2 gap-4 mb-6">
        {currentQ.options?.map((opt, i) => (
          <button
            ...
            onClick={() => handleAnswer(i)} // ✅ Add answer handling
          >
            {opt}
          </button>
        ))}
